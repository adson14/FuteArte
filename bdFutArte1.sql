...
INSERT INTO FUNCIONARIO (nome, idade, rg,cpf,endereco,cargo)
 ...
 SELECT  sum(valor)   FROM MENSALIDADE M INNER JOIN ALUNO A ON(M.ALUNO_cod_Aluno=A.cod_Aluno) WHERE TURMA_cod_Turma ='2';
 
 SELECT  A.nome,M.dataVencimento,M.valor FROM MENSALIDADE M INNER JOIN ALUNO A ON(M.ALUNO_cod_Aluno=A.cod_Aluno) WHERE TURMA_cod_Turma ='2';
...
SELECT  sum(valor)   FROM MENSALIDADE M INNER JOIN ALUNO A ON(M.ALUNO_cod_Aluno=A.cod_Aluno) WHERE TURMA_cod_Turma =1


SELECT T.cod_Turma, T.nome AS 'TurmaNome',T.FUNCIONARIO_cod_Func, A.cod_Aluno, A.nome, A.idade, A.rg, A.cpf, A.endereco, A.statusMedico, A.FUNCIONARIO_cod_Func, A.TURMA_cod_Turma FROM Turma T INNER JOIN Aluno A ON A.TURMA_cod_Turma = T.cod_Turma where T.cod_Turma = 1;